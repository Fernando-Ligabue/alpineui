...
function looseCompareArrays2(a, b) {
  if (a.length !== b.length) return false;
  let equal = true;
  for (let i = 0; equal && i < a.length; i++) {
    equal = looseEqual2(a[i], b[i]);
  }
  return equal;
}
function looseEqual2(a, b) {
  ...
  if (aValidType || bValidType) {
    return aValidType && bValidType ? looseCompareArrays2(a, b) : false;
  }
  ...
  if (aValidType || bValidType) {
    ...
    for (const key in a) {
      const aHasKey = a.hasOwnProperty(key);
      const bHasKey = b.hasOwnProperty(key);
      if (aHasKey && !bHasKey || !aHasKey && bHasKey || !looseEqual2(a[key], b[key])) {
        return false;
      }
    }
  }
...
function looseIndexOf(arr, val) {
  return arr.findIndex((item) => looseEqual2(item, val));
}
...
var patchProp = (el, key, prevValue, nextValue, namespace, parentComponent) => {
  ...
  if (key === "class") {
    ...
    el._def.__asyncLoader && (/[A-Z]/.test(key) || !isString2(nextValue)))
  ) {
...
function shouldSetAsPropForVueCE(el, key) {
  ...
  return Array.isArray(props) ? props.some((prop) => camelize2(prop) === camelKey) : Object.keys(props).some((prop) => camelize2(prop) === camelKey);
}
var REMOVAL = {};
...
function castValue(value, trim, number) {
  ...
  if (number) value = looseToNumber2(value);
  return value;
}
var vModelText = {
...
function setChecked(el, { value, oldValue }, vnode) {
  ...
  if (isArray2(value)) {
    ...
    if (value === oldValue) return;
    checked = looseEqual2(value, getCheckboxValue(el, true));
  }
...
var vModelRadio = {
  created(el, { value }, vnode) {
    el.checked = looseEqual2(value, vnode.props.value);
    el[assignKey] = getModelAssigner(vnode);
    addEventListener(el, "change", () => {
...
function setSelected(el, value) {
  ...
  for (let i = 0, l = el.options.length; i < l; i++) {
    ...
    } else if (looseEqual2(getValue(option), value)) {
...
function initVModelForSSR() {
  ...
  vModelRadio.getSSRProps = ({ value }, vnode) => {
    if (vnode.props && looseEqual2(vnode.props.value, value)) {
      return { checked: true };
    }
  };
...
vue/dist/vue.runtime.esm-bundler.js:
  ...
  **)
*/
//# sourceMappingURL=chunk-R3FFKKH3.js.map
